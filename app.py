...
def show_venue(venue_id):
    ...
    upcoming_shows = venue.shows.filter(Show.start_time >= datetime_now).all()
...
def show_artist(artist_id):
    ...
    datetime_now = datetime.now().replace(tzinfo=pytz.utc)
    upcoming_shows = artist.shows.filter(Show.start_time >= datetime_now).all()
    upcoming_shows_count = len(upcoming_shows)
    past_shows = artist.shows.filter(Show.start_time < datetime_now).all()
    past_shows_count = len(past_shows)
